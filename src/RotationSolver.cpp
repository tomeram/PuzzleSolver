...
bool RotationSolver::fillRow(int rowSize, int colSize, int row , int col_left , int col_right, RotationSolution sol, TypesMap &typesMap) {
	if (col_left > col_right) {
		...
		for (auto type : list) {
			...
			if (piece != nullptr) {
				...
				bool b= fillRow(rowSize, colSize, row , col_left+1 , col_right , sol, newMap);
				if (b == true) {
					return true;
				}
				else{
...
bool RotationSolver::topRow(int rowSize, int colSize, int row_top , int col_left , int col_right, RotationSolution sol, TypesMap &typesMap) {
	return fillRow(rowSize, colSize, row_top , col_left , col_right, sol, typesMap);
}

bool RotationSolver::bottomRow(int rowSize, int colSize, int row_bott , int col_left , int col_right, RotationSolution sol, TypesMap &typesMap) {
	return fillRow(rowSize, colSize, row_bott , col_left , col_right, sol, typesMap);
}
...
bool RotationSolver::fillFrames(int rowSize, int colSize , int row_top, int row_bott,
	...
	if (col_left < col_right && row_top < row_bott) {
		if (topRow(rowSize, colSize, row_top , col_left , col_right, sol, typesMap) &&
				bottomRow(rowSize, colSize, row_bott , col_left , col_right, sol, typesMap) &&
	...
	if (col_left < col_right && row_top == row_bott) {
		if (leftCol(rowSize, colSize, col_left , row_top , row_bott, sol, typesMap) &&
				rightCol(rowSize, colSize, col_right, row_top, row_bott, sol, typesMap) &&
				topRow(rowSize, colSize, row_top , col_left , col_right, sol, typesMap)) {
